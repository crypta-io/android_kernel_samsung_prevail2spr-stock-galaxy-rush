...
#elif defined(CONFIG_SENSORS_K2DM)
#include "yas_acc_driver-k2dm.c"
#elif defined(CONFIG_SENSORS_KR3DM)
#include "yas_acc_driver-kr3dm.c"
#endif
#else
...
char vendor_name[MAX_CHIP_NUM][MAX_LEN_OF_NAME] = {
	...
	{"STM"},
#endif
#ifdef CONFIG_SENSORS_K2DM
	{"STM"},
#endif
#ifdef CONFIG_SENSORS_KR3DM
	{"STM"},
#endif
...
/*	yas_acc_set_offset(data->driver, &data->cal_data);*/
...
#endif
	err = driver->set_position(CONFIG_SENSORS_YAS_ACCEL_POSITION);
	if (err != YAS_NO_ERROR) {
		kfree(driver);
		return err;
	}
...
static int yas_acc_set_enable(struct yas_acc_driver *driver, int enable)
{
	...
	if (yas_acc_get_enable(data->driver) != enable) {
		if (enable) {
			if (yas_pdata && yas_pdata->vreg_en)
				yas_pdata->vreg_en(1);
			else
				pr_info("%s : enable error\n", __func__);
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d, %d, %d\n",
		x * CHIP_RESOLUTION / HAL_RESOLUTION ,
		y * CHIP_RESOLUTION / HAL_RESOLUTION ,
		z * CHIP_RESOLUTION / HAL_RESOLUTION);
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (yas_pdata && yas_pdata->vreg_en)
		yas_acc_set_enable(data->driver, 0);
	return 0;
